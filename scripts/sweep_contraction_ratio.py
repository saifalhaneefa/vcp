"""Sensitivity analysis for the progressive contraction-shrinking rule.

Does not change the baseline configuration. For each max_contraction_ratio,
counts structural VCP days, fresh setups, breakout candidates and complete
signals.

Example:
    python scripts/sweep_contraction_ratio.py
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Comma-separated max_contraction_ratio values. Lower is stricter.",
    )
    ...
    for ratio in ratios:
        if ratio <= 0:
            raise SystemExit("Each ratio must be > 0.")
        cfg = replace(
            base,
            vcp=replace(base.vcp, max_contraction_ratio=ratio),
        )
        stages = count_stages(data, cfg, start, end)
        rows.append({
            "max_contraction_ratio": ratio,
            **stages,
        })
    ...
    print("=" * 100)
    print("PROGRESSIVE CONTRACTION RATIO SENSITIVITY")
    print("=" * 100)
    print(f"Period: {args.start} -> {args.end}")
    print(f"Symbols: {len(data)}")
    print("Current baseline ratio:", base.vcp.max_contraction_ratio)
    print(report.to_string(index=False))

    out = PROJECT_ROOT / "reports"
    out.mkdir(exist_ok=True)
    path = out / "contraction_ratio_sensitivity.csv"
